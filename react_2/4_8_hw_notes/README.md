# React Module 2 PWA

## Project: Note

#### [home page]

![home](public/assets/screenshots/home.png)

#### [register page]

![register in](public/assets/screenshots/register.png)

#### [log in page]

![log in](public/assets/screenshots/login.png)

#### [log in page]

![log in error](public/assets/screenshots/login-error.png)

#### [add 1 page]

![add](public/assets/screenshots/add-note-1.png)

#### [add 2 page]

![add](public/assets/screenshots/add-note-2.png)

#### [add 3 page]

![add-1](public/assets/screenshots/add-note-3.png)

#### [edit page]

![edit](public/assets/screenshots/edit.png)

#### [list page]

![list](public/assets/screenshots/list-note.png)

#### [search page]

![search](public/assets/screenshots/search.png)

#### [manifest]

![manifest](public/assets/screenshots/manifest.png)

#### [sw]

![sw](public/assets/screenshots/sw.png)

#### [local storage]

![local storage](public/assets/screenshots/local-storage.png)

#### [db]

![db](public/assets/screenshots/db.png)